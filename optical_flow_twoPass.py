...
# =================================================================================================
# OPTICAL FLOW TWO-PASS PIPELINE — EXECUTION MODES (Step 2 Bifurcation Overview)
#
# This pipeline now supports **three explicit execution modes**, selected via CLI flags.
# Only ONE mode runs per invocation. If no flags are provided, the pipeline defaults to
# the original full OF + P1 + P2 behavior.
#
# -------------------------------------------------------------------------------------------------
# MODE 1 — PRECOMPUTE-OF (heavy run, one-time)
#   Flag:  --precompute-of
#
#   • Runs the full optical-flow pipeline (P1 + P2) once.
#   • Collects per-frame motion metrics (active_gated pixel counts).
#   • Saves metrics to disk for later reuse.
#   • Exits early after caching; does NOT run segmentation or scoring.
#
#   Use when preparing for large ML sweeps or repeated scoring runs.
#
# -------------------------------------------------------------------------------------------------
# MODE 2 — FAST SWEEP (segmentation-only, no optical flow)
#   Flag:  --use-cached-of
#
#   • Loads cached motion metrics from PRECOMPUTE-OF mode.
#   • Applies current config/override thresholds.
#   • Performs segmentation-only (P1-style) logic.
#   • Rebuilds segments and computes final score.
#   • Runs in ~0.2–0.5 seconds per clip (no OF computation).
#
#   Use for ML sweeps, hyperparameter tuning, or rapid scoring.
#
# -------------------------------------------------------------------------------------------------
# MODE 3 — FULL PIPELINE MODE (default)
#   No flags required.
#
#   • Original behavior: compute optical flow, run P1, run P2 recovery,
#     rebuild final segments, export packets, compute final score.
#   • This mode is unchanged by the bifurcation architecture.
#
#   Use for production runs, diagnostics, and packet export.
#
# -------------------------------------------------------------------------------------------------
# Summary:
#   --precompute-of     → heavy run, cache OF metrics, exit
#   --use-cached-of     → fast segmentation-only scoring using cached metrics
#   (no flags)          → full OF + P1 + P2 pipeline (original behavior)



# ML_OpticalFlow_Data_Collection_Driver.py

# This script is not a mode of the pipeline.
# It is the automation layer that repeatedly calls Mode 2.

# Its job is:

# ✔ Sweep through many override combinations
# ✔ Call optical_flow_twoPass.py with --use-cached-of
# ✔ Capture the [RESULT] score=… output
# ✔ Save each run as a row of ML training data
#
# =================================================================================================
...
# from motion_core.filtering import (
    ...
    # build_water_mask,
# )
# from motion_core.segmentation import segment_from_kept
from motion_core.components import compute_flow_and_mag
from motion_core.components import segment_from_kept
...
AUTO_TUNER_ENABLED = True        # set True or False to use
...
class Config:
    ...
    # GATING PRIMITIVES
    # ---------------------------------------------------------
    # These toggles control the *normally ordered* PASS‑1 gating
    # pipeline. When all toggles are True, the gating sequence
    # follows the standard production order:
    #
    #   1. threshold_mag
    #   2. apply_keep_mask
    #   3. apply_roi_mask
    ...
    #   5. apply_morphology
    #
    # This ordering is unchanged from the canonical CLI pipeline.
    # Disabling any toggle removes that step from the sequence,
    # enabling controlled ablations and config sweeps for ML.
    #
    # NOTE:
    # - "Normally ordered" means the steps execute in the order
    #   listed above unless explicitly disabled.
    # - "Unchanged" means the default behavior matches the CLI
    #   production pipeline with all gating primitives enabled.
    # ---------------------------------------------------------
    GATING: dict = field(default_factory=lambda: {
        "USE_THRESHOLD": True,
        "USE_KEEP_MASK": True,
        "USE_ROI_MASK": True,
        "USE_WATER_MASK": True,
...
class State:
    ...
    # ---------------------------------------------------------
    # Step 3: motion-metric collection fields - Bifurcation
    # ---------------------------------------------------------
    collect_motion_metrics: bool = False
    motion_pixels_per_frame: List[int] = field(default_factory=list)
...
def detect_motion_frames(
    ...
    roi_mask = build_roi_mask(config, (frame_h, frame_w))
    water_mask = build_water_mask(config, (frame_h, frame_w))
    ...
    for idx in range(1, total_frames):
        ...
        # --------------------------------------------------------------
        # PASS 1: optical flow + gating pipeline
        # --------------------------------------------------------------
        ...
        # 2. keep mask (ROI + water already baked in)
...
def p2_recover_and_rebuild_segments(
    ...
    for (s, e) in p1_segments:
        ...
        for idx in range(win_s + 1, win_e + 1):
            ...
            raw_mask = threshold_mag(mag, thr2)
...
def prescan(cap, config, preset, num_frames=40, low_flow_thresh=0.5):
    """
    Lightweight optical-flow prescan on the first N frames.
    Returns metrics used by classify_target().
    """

    # ------------------------------------------------------------
    # Precompute masks ONCE (ROI + water already combined in keep_mask)
    # ------------------------------------------------------------
    keep_mask = build_keep_mask(config, (int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)),
                                         int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))),
                                preset)
    keep_mask_bool = (keep_mask == 255)
    ...
    while frame_count < num_frames:
        ...
        if prev_gray is not None:
            ...
            # p95 magnitude
            flow_mags.append(np.percentile(mag, 95))

            # Apply combined mask (ROI + water)
            mag_masked = mag * keep_mask_bool

            # Threshold
            active_mask = mag_masked > low_flow_thresh
            ...
            # Flow energy
            flow_energy_vals.append(float(mag_masked.mean()))
...
def process_clip(video_path: Path, paths: Dict[str, Any], config: Config, state: State) -> None:
    ...
    # GS Bifurcation
    p1_segments, kept_p1, _ = detect_motion_frames(
        video_path,
        config,
        preset,
        state,      # ← NEW
    ...
    if final_segments:
        ...
        for i, (start, end) in enumerate(final_segments, 1):
            ...
            for fidx in range(start + 1, end + 1):
                ...
                # module changes  here
                mag, _ = cv2.cartToPolar(flow[..., 0], flow[..., 1])
                motion_mask = threshold_mag(mag, float(config.MOTION_THRESHOLD))
                if config.MASK_GATING_PASS2 == "hard":
                    motion_mask = apply_keep_mask(motion_mask, keep_mask)
...
# ---------------------------------------------------------
# Step 3: Precompute-OF helpers - Bifurcation
#  This function is a hook - not a second copy of your frame loop.
# ---------------------------------------------------------

# Where cached metrics will be stored
CACHED_METRICS_PATH = VIDEO_ROOT / "of_cached_metrics.json"


def save_cached_metrics(motion_pixels_per_frame: list[int]) -> None:
    """
    Persist per-frame motion metrics for fast segmentation-only sweeps.
    """
    payload = {
        "motion_pixels_per_frame": motion_pixels_per_frame,
    }

    with open(CACHED_METRICS_PATH, "w", encoding="utf-8") as f:
        json.dump(payload, f)

    print(f"[CACHE] Saved cached metrics to: {CACHED_METRICS_PATH}")



def run_full_of_and_collect_metrics(
    video_path: Path,
    preset: str,
    config: Config,
    state: State,
    ...
    """
    Step 3: Run the existing OF P1 path once for a single clip,
    while collecting per-frame motion pixel counts via the
    detect_motion_frames() instrumentation.

    This does NOT re-implement optical flow. It simply calls the
    same P1 function the full pipeline uses, with the Step 3 splice
    active inside detect_motion_frames().
    """

    # Enable metric collection for this run
    state.collect_motion_metrics = True
    state.motion_pixels_per_frame = []

    # Call the same P1 function the full pipeline uses
    _segments, _kept_p1, _total_frames = detect_motion_frames(
        video_path,
        config,
        preset,
        state,
    ...
    # Metrics were appended inside detect_motion_frames()
    return state.motion_pixels_per_frame




# ---------------------------------------------------------
# Step 4: Fast Sweep Mode (segmentation-only)  - Bifurcation
# ---------------------------------------------------------

def load_cached_metrics() -> list[int]:
    """
    Load cached per-frame motion metrics saved during precompute-of mode.
    """
    if not CACHED_METRICS_PATH.exists():
        raise FileNotFoundError(
            f"No cached metrics found at {CACHED_METRICS_PATH}. "
            "Run with --precompute-of first."
        )

    with open(CACHED_METRICS_PATH, "r", encoding="utf-8") as f:
        payload = json.load(f)

    return payload.get("motion_pixels_per_frame", [])
   

def run_segmentation_only_fast_path(config: Config, state: State) -> float:
    """
    Fast path: segmentation-only scoring using cached motion metrics.
    Returns the final score (float).
    """

    # Load cached per-frame motion pixel counts
    motion_pixels_per_frame = load_cached_metrics()

    # ---------------------------------------------------------
    # Apply segmentation logic using current config thresholds
    # ---------------------------------------------------------
    kept_frames = []
    thr = int(config.MIN_MOTION_PIXELS)

    for idx, pix in enumerate(motion_pixels_per_frame):
        if pix > thr:
            kept_frames.append(idx)

    # Build segments from kept frames
    segments = _segments_from_kept_frames(kept_frames, config)

    # ---------------------------------------------------------
    # Compute score (same scoring logic your pipeline uses)
    # ---------------------------------------------------------
    if not segments:
        return 0.0

    # Example scoring: total active frames
    total_active = sum((e - s + 1) for (s, e) in segments)

    return float(total_active)
...
def main() -> None:
    ...
    # ---------------------------------------------------------
    # NEW: dual‑mode pipeline flags (Step 2 bifurcation)
    # ---------------------------------------------------------
    parser.add_argument(
        "--precompute-of",
        action="store_true",
        help="Run full optical flow once and cache per-frame motion metrics",
    )

    parser.add_argument(
        "--use-cached-of",
        action="store_true",
        help="Skip optical flow and use cached metrics for fast segmentation-only scoring",
    )
    ...
    # ---------------------------------------------------------
    # Apply pipeline-level output toggles
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # APPLY OVERRIDES (shared by FAST SWEEP and FULL PIPELINE)
    # ---------------------------------------------------------
    ...
    if raw_cli and raw_cli not in ("{}", ""):
        ...
        if isinstance(override_dict, dict) and override_dict:
            ...
            state.override_info = apply_overrides(
                config,
                override_dict,
                "command-line",
    ...
    # ---------------------------------------------------------
    # MODE SELECTION (Step 2) Bifurcation
    #  Three execution paths: PRECOMPUTE‑OF, FAST SWEEP, FULL PIPELINE
    # ---------------------------------------------------------

    # MODE 1: PRECOMPUTE-OF (one-time heavy run)
    if args.precompute_of:
        print("[MODE] PRECOMPUTE-OF: computing optical flow once and caching metrics")

        preset_name = state.requested_preset or args.preset or "Home"

        motion_pixels_per_frame = run_full_of_and_collect_metrics(
            first_clip,
            preset_name,
            config,
            state,
        )
        save_cached_metrics(motion_pixels_per_frame)

        print("[MODE] PRECOMPUTE-OF complete.")
        return

    # MODE 2: FAST SWEEP MODE (segmentation-only)
    if args.use_cached_of:
        print("[MODE] FAST SWEEP: using cached OF metrics for segmentation-only scoring")

        score = run_segmentation_only_fast_path(config, state)

        print(f"[RESULT] score={score}")
        return

    # MODE 3: DEFAULT FULL PIPELINE MODE (current behavior)
    print("[MODE] FULL PIPELINE MODE (default)")

    # ---------------------------------------------------------
    # Full pipeline: process all videos
    # ---------------------------------------------------------
